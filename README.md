...
이 레포지토리는 전체 Unity 프로젝트를 그대로 보여주기보다, 정해진 스토리 진행을 유지하면서도 각 스테이지가 자기만의 퍼즐을 가질 수 있게 하려 했던 **3주 1인 개발의 선택과 한계**를 정리합니다.
...
DualMind에서 가장 중요하게 본 것은 “퍼즐 기능을 많이 만드는 것”보다, **스토리 진행을 통제하면서도 각 스테이지가 독립적인 경험을 만들 수 있는 구조**였습니다. 처음에는 공통 흐름을 상속으로 묶는 것이 가장 빠르고 명확한 선택이었지만, 이후 시스템을 더 붙일 때 절차가 Stage에 모이는 한계도 경험했습니다.
...
|---|---|---|---|
| Stage Sequence | 스테이지마다 진행 흐름을 따로 구현하면 공통 제어가 흩어짐 | 공통 흐름은 상속으로 묶고 스테이지별 차이는 가상 코루틴으로 구현 | [Stage Sequence](docs/systems/stage-sequence.md) |
